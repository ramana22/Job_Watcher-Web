...
public class ApplicationsController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<ApplicationResponse>>> UpsertApplications([FromBody] IEnumerable<ApplicationCreateRequest> payload)
    {
        ...
        foreach (var item in payload)
        {
            ...
            var calculatedScore = _matchingService.CalculateScore(resumeText, existing.JobTitle, existing.Description, existing.SearchKey);
            if (!string.IsNullOrWhiteSpace(resumeText))
            {
                existing.MatchingScore = calculatedScore;
            }
            else if (item.MatchingScore.HasValue)
            {
                existing.MatchingScore = item.MatchingScore.Value;
            }
